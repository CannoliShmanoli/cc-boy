...
function Timers.new(modules)
  ...
  function timers:update()
    if timers.timer_enabled then
      local ram = io.ram
      local rate_select = bit32.band(ram[io.ports.TAC], 0x3)
      local rate = timers.clock_rates[rate_select]
      local timer_offset = timers.timer_offset
      local system_clock = timers.system_clock
      while system_clock > timer_offset + rate do
        ram[io.ports.TIMA] = bit32.band(ram[io.ports.TIMA] + 1, 0xFF)
        timer_offset = timer_offset + rate
        if ram[io.ports.TIMA] == 0x00 then
          --overflow happened, first reset TIMA to TMA
          ram[io.ports.TIMA] = ram[io.ports.TMA]
          --then, fire off the timer interrupt
          interrupts.raise(interrupts.Timer)
        end
      end
      timers.timer_offset = timer_offset
    end
  end
