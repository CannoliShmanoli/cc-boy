...
local Cache = require("graphics/cache")
local Palette = require("graphics/palette")
local Registers = require("graphics/registers")
...
function Graphics.new(modules)
  ...
  graphics.reset = function()
    ...
    -- Sync IO register write logic (io.reset set values directly, bypassing handlers)
    if io.write_logic[ports.LCDC] then io.write_logic[ports.LCDC](io.ram[ports.LCDC]) end
    if io.write_logic[ports.STAT] then io.write_logic[ports.STAT](io.ram[ports.STAT]) end
    if io.write_logic[ports.BGP] then io.write_logic[ports.BGP](io.ram[ports.BGP]) end
    if io.write_logic[ports.OBP0] then io.write_logic[ports.OBP0](io.ram[ports.OBP0]) end
    if io.write_logic[ports.OBP1] then io.write_logic[ports.OBP1](io.ram[ports.OBP1]) end
  end
  ...
  graphics.load_state = function(state)
    ...
    for i = 0, 39 do
      graphics.cache.refreshOamEntry(i)
    end
  ...
  graphics.draw_next_pixels = function(duration)
    ...
    while scanline_data.x < duration and scanline_data.x < 160 do
      ...
      if graphics.registers.background_enabled then
        ...
        local active_palette = scanline_data.active_attr.palette[bg_index + 1]

        game_screen[ly][dx][1] = active_palette[3]
        game_screen[ly][dx][2] = active_palette[2]
        game_screen[ly][dx][3] = active_palette[1]
      end
  ...
  graphics.draw_sprites_into_scanline = function(scanline, bg_index, bg_priority)
      ...
      if scanline >= sprite_lower and scanline < sprite_upper then
        if active_count < 10 then
          ...
          for j = 1, active_count do
            local lowest_x = oam_cache[lowest_priority].x
    ...
    for i = active_count, 1, -1 do
      local sprite = oam_cache[active_sprites[i]]
        ...
        if display_x >= 0 and display_x < 160 then
          ...
          if subpixel_index > 0 then
            if (bg_priority[display_x] == false and not sprite.bg_priority) or bg_index[display_x] == 0 or graphics.registers.oam_priority then
              local subpixel_color = sprite.palette[subpixel_index + 1]
