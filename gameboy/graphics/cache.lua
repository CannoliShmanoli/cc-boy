...
function Cache.new(graphics)
  ...
  local function use_cgb_palettes()
    local gameboy = graphics.gameboy
    return gameboy and gameboy.type == gameboy.types.color and
      gameboy.cartridge and gameboy.cartridge.header and gameboy.cartridge.header.color
  end
  ...
  cache.reset = function()
    ...
    for i = 0, 39 do
      ...
      if use_cgb_palettes() then
        ...
        cache.oam[i].palette = graphics.palette.obj0
      end
    end
  end
  ...
  cache.refreshAttributes = function(map_attr, x, y, address)
    ...
    if use_cgb_palettes() then
      ...
      map_attr[x][y].bank = 0
      map_attr[x][y].horizontal_flip = false
      map_attr[x][y].vertical_flip = false
      map_attr[x][y].priority = false
    end
  end
