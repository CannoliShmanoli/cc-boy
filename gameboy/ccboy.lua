...
local screen_w = cols * 6
local screen_h = rows * 9
...
local function draw_game_screen()
    ...
    term.drawPixels(ox, oy, pixels_indexed)
end
...
local function draw_border()
    ...
    oy = default_oy
    if not border then return end
    term.drawPixels(border_ox, border_oy, border_indexed)
end
...
local menu_items = {" Settings   ", " Save State ", " Load State ", " Quit Game  "}
        ...
        elseif k == keys.enter then
            ...
            elseif sel == 2 then
                local ok, err = pcall(function()
                    save_battery()
                    local state = gb.save_state()
                    local s = textutils.serialize(state)
                    local f = fs.open(state_path, "w")
                    f.write(s)
                    f.close()
                end)
                if ok then msg, msg_color = "Saved!", colors.green
                else msg, msg_color = "Save err: " .. tostring(err), colors.red end
                draw()
        elseif sel == 3 then
            if fs.exists(state_path) then
                ...
                local d = f.readAll()
                f.close()
                local ok, st = pcall(textutils.unserialize, d)
                if ok and st then
                    local ok2, err2 = pcall(function() gb.load_state(st) end)
                    if ok2 then
                        frame_cycle_target = gb.timers.system_clock + CYCLES_PER_FRAME
                        last_vblank = gb.graphics.vblank_count
                        frame_count = 0
                        msg, msg_color = "Loaded!", colors.green
                    else msg, msg_color = "Load err: " .. tostring(err2), colors.red end
                else
                    msg, msg_color = "Corrupt save", colors.red
                end
            else
                msg, msg_color = "No save file", colors.red
            end
                draw()
            elseif sel == 4 then
                save_battery()
