local rom_header = require("rom_header")

local MbcNone = require("mbc/none")
local Mbc1 = require("mbc/mbc1")
local Mbc2 = require("mbc/mbc2")
local Mbc3 = require("mbc/mbc3")
local Mbc5 = require("mbc/mbc5")
...
function Cartridge.new(modules)
  ...
  cartridge.load = function(file_data, size)
    ...
    cartridge.external_ram_size = cartridge.header.ram_size or 0
    if cartridge.header.mbc_type == 0x05 or cartridge.header.mbc_type == 0x06 then
      cartridge.external_ram_size = 512
    end
  ...
  cartridge.load_external_ram = function(data)
    if not data then return end
    local size = math.min(#data, cartridge.external_ram_size)
    for i = 0, size - 1 do
      cartridge.external_ram[i] = data:byte(i + 1) or 0
    end
    cartridge.external_ram.dirty = false
  end

  cartridge.dump_external_ram = function()
    local size = cartridge.external_ram_size or 0
    if not size or size <= 0 then size = 128 * 1024 end
    local out = {}
    for i = 0, size - 1 do
      out[i + 1] = string.char(cartridge.external_ram[i] or 0)
    end
    cartridge.external_ram.dirty = false
