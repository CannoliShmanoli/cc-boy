...
local function show_settings_menu()
    ...
    local function draw()
        ...
        local render = settings.render_mode == "pixelbox" and "Pixelbox" or "Graphics"
        local items = {"CGB Bypass  [" .. cgb .. "]", "Rendering Mode [" .. render .. "]", "Back"}
        num_items = #items
        local width = 20
        for i = 1, #items do
            if #items[i] > width then width = #items[i] end
        end
        local cx = math.floor((cols - width - 2) / 2) + 1
        ...
        for i = 1, #items do
            ...
            term.write(" " .. items[i] .. string.rep(" ", width - #items[i]) .. " ")
        end
    ...
    while true do
        ...
        elseif k == keys.down and sel < num_items then sel = sel + 1; draw()
        elseif k == keys.enter then
            if sel == 1 then
                ...
                draw()
            elseif sel == 2 then
                settings.render_mode = settings.render_mode == "graphics" and "pixelbox" or "graphics"
                save_settings()
                draw()
            else
                term.setBackgroundColor(colors.black)
